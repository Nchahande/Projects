$(function () {
    ...
    $('.date-field-dob').datepicker({
        ...
        if (Math.sign(age) == -1) {
            ...
            age = 0;
            $(this).next('.selected-years').css('display','none');
        }
        else{
        ...
        if (age == 1) {
            $(this).next('.selected-years').html(age + ' year');
        }
        else {
            $(this).next('.selected-years').html(age + ' years');
        }
    ...
    $('.feedback_blocks ul li').click(function(){
        var value = $(this).text();
        $('.feedback_blocks ul li').removeClass('active').css('backgroundColor', "#f4f6fa");
        if (value <= 6) {
...
function validateName(nameval){
    var values = nameval.split(' ').filter(function(v){return v!==''});
                if (values.length > 1) {
                    // var correct=$(this).siblings('p.error-msg');
                    // correct.css('display', 'none');
                    return true;
